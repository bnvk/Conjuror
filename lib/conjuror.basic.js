...
Conjuror.magickData = function(data, schema, date) {
  ...
  var increment_output = function(item) {
    ...
    // HTML format
    if (_.indexOf(Conjuror.args.options.format, 'html') > -1 || _.indexOf(Conjuror.args.options.format, 'pdf') > -1) {
      outputs.html += '<tr>\n';
...
Conjuror.castToHTML = function(outputs, user, resource){
  ...
  if (_.indexOf(Conjuror.args.options.format, 'html') > -1 || _.indexOf(Conjuror.args.options.format, 'pdf') > -1) {
    ...
    // Get HTML Template
    var template = Conjuror.args.config.invoice_template || 'invoice';
    var template_path = Conjuror.args.app_path + 'templates/' + template + '.html';
    ...
    Conjuror.readManuscript(template_path)
      ...
      .then(function(buffer) {
        ...
        // Save HTML file
        if (_.indexOf(Conjuror.args.options.format, 'html') > -1) {
...
Conjuror.castHTMLToPDF = function(output_html, output_name){
  if (_.indexOf(Conjuror.args.options.format, 'pdf') > -1) {
    ...
    .then(function(buffer) {
      ...
      csv.parse(data, function(err, data) {
        ...
        Conjuror.getClient(client_path, Conjuror.args.options.trim, function(client) {
          ...
          if (Conjuror.args.options.format) {
            console.log('-----------------------------------------------------------------------------');
            console.log('Output Formats: ' + Conjuror.args.options.format.join(','));
          }
          ...
          console.log('Total monies earned: ' + (Conjuror.args.options.currency || '$') + outputs.totals.money);
...
// Load Schema
Conjuror.Grow = function(schema_file, args) {
  Conjuror.args = args;
  ...
  Conjuror.readManuscript(schema_file)
    .then(function(buffer) {
      ...
      console.log("Error while growing", error);
    });
};
