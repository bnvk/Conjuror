...
Conjuror.getIngredients = function(config_file, callback){
  ...
  Conjuror.readManuscript(config_file)
    .then(function(buffer) {
      ...
      console.log('You don\'t have a config file. Run "node setup.js" to create one');
    ...
    .then(function(buffer) {
      ...
      Conjuror.readManuscript(dataPath + '/' + clients_json[0].resources[0].path)
        .then(function(buffer) {
          ...
          console.log("Error while reading client json manuscript", error);
          return callback(error);
        })
    }).catch(function(error) {
      console.log("Error while reading schema file", error);
      return callback(error);
    });
  // });
}
